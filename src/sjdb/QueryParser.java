 ...
 * 
 * SELECT <attribute name>,<attribute name>,...,<attribute name> FROM <relation
 * name>,<relation name>,...,<relation name> WHERE
 * <predicate>,<predicate>,...,<predicate>
 * 
 * where <predicate> is of one of the following two forms:
 * 
 * <attribute name>="<value>" <attribute name>=<attribute name>
 * 
 * The WHERE line (corresponding to the select operators) is optional and may be
 * omitted; the other lines are required.
 * 
 * To form the canonical query, a left-deep tree of cartesian products over
 * scans over the named relations is built, following by a series of select with
 * the given predicates, and then a single project with the given attributes.
 * 
 * Note that the author of this class was extremely lazy, and so the parsing is
 * unforgiving and may be sensitive to extraneous whitespace. In particular,
...
public class QueryParser {
	...
	/**
	 * Read a query from the input (via the BufferedReader) and parse it to create a
	...
	/**
	 * Build a left-deep cartesian product tree from the relations with the given
	 * names
	 * 
	 * @param names
	 * @return
	...
	/**
	 * Build a scan operator that reads the relation with the given name
	 * 
	 * @param name
	 * @return
	 */
